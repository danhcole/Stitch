...
\lstset{escapechar=@,style=customc,language=C}
\lstset{
 morekeywords={let, var, string, bool, def, async}
}
...
In addition to the standard C primitive types (\verb|int, double, char|, etc.), Stitch has native support for the \verb|string| type.  This includes concatenation, and a inbuilt length operator.  Stitch also has support for the \verb|bool| type.\\
